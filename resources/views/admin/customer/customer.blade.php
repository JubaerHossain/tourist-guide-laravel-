						...
						<tbody>
							...
							<tr>
								...
								<td>
                                        ...
                                        {{ asset('customer/profile/'.$c->profile_pic) }}
                                        @else
                                        {{asset('assets/frontend/images/avatar.jpg')}}
                                        @endif" alt="Profile" width="70"/>
                                </td>
								<td>
									<button onclick="Approve({{$key}})" class="btn btn-sm {{$c->verified_at  == 0 ?'btn-info':'btn-warning'}}">
										<i class="{{$c->verified_at == 0 ?'fas fa-check':'fas fa-times'}}"></i>
									</button>
									
								  <form id="app-form-{{ $key }}" action="{{ route('admin.customer_approve', $c->user->id) }}" method="POST" style="display: none;">
										@csrf
										@method('POST')
									 </form>
									 <a class="btn btn-primary btn-sm" href="{{route('admin.customer_view',$c->id)}}"><i class="fa fa-eye"></i></a>
									 <button onclick="delePro({{$key}})" class="btn btn-danger btn-sm">
										<i class="fa fa-trash-o" aria-hidden="true"></i>
									 </button>
									 <form id="delete-form-{{ $key }}" action="{{ route('admin.customer_delete', $c->user->id) }}" method="POST" style="display: none;">
										@csrf
										@method('DELETE')
									 </form>
								</td>
