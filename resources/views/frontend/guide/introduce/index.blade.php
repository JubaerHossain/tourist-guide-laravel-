            ...
            <div class="row">                
                <div class="col-md-2 pb-5">
                    <div class="list-group" id="list-tab" role="tablist">
                        ...
                        <a class="list-group-item pro-guide list-group-item-action active" href="{{route('guide.introduce_profile')}}" role="tab">Introduce</a>
                        ...
                        <table class="table table-bordered">
                                <thead>
                                  <tr>
                                    ...
                                    <th scope="col">Vedio link</th>
                                    <th scope="col">Action</th>
                                  </tr>
                                </thead>
                                <tbody>   
                                    @foreach (@Auth::user()->guide->guide_videos as $key => $item) 
                                  <tr>
                                    <td>{{ str_limit($item->title,40) }}</td>                                      
                                    <td>
                                        <img align="left" class="" src="
                                            @if($item->image != null)
                                            {{ asset('guide/introduce/'.$item->image) }}
                                            @else
                                            {{asset('assets/frontend/images/image_1.jpg')}}
                                            @endif" alt="Profile" width="70"/>
                                    </td>
                                    <td>{{ $item->guide_video }}</td>
                                        <td>
                                        ...
                                        <form id="delete-form-{{ $key }}" action="{{ route('guide.delete_introduce_profile', $item->id) }}" method="POST" style="display: none;">
                                                @csrf
                                                @method('DELETE')
                                            </form>
                                        </td>
            ...
            <div class="modal-dialog" role="document">
              <div class="modal-content passions">
                ...
                <div class="modal-body">
                        <form  action="{{route('guide.add_introduce_profile')}}" enctype="multipart/form-data" method="POST">
                            @csrf
                            ...
                            <div class="form-group">     
                              <label for="name">Vedio link*</label><br>
        ...
        <div class="modal fade" id="edit{{$item->id}}" tabindex="-1" role="dialog" aria-labelledby="edit" aria-hidden="true">
            <div class="modal-dialog" role="document">
              <div class="modal-content passions">
                ...
                <div class="modal-body">
                        <form  action="{{route('guide.edit_introduce_profile',$item->id)}}" enctype="multipart/form-data" method="POST">
                            @csrf
                            <div class="form-group">
                                    <label for="name">Title*</label><br>
                            ...
                            <div class="form-group">
                                    <label for="name">Video link*</label><br>
                                    <input type="text" class="form-control" name="link"  value="{{ $item->guide_video }}">                                
                            </div> 
